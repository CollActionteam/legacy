...
resource "aws_alb_target_group" "testselect-collaction" {
...
resource "aws_lb_listener_rule" "testselect-collaction" {
  listener_arn = data.aws_alb_listener.api-collaction.arn

  action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.testselect-collaction.arn
  }
...
resource "aws_route53_record" "testselect-collaction" {
