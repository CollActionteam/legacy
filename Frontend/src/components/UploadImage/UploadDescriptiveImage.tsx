...
import UploadImage, { IUploadImageState } from ".";
...
> {
  ...
  render() {
    return (
      <div id="project-image">
        <div
          className="col-xs-12 col-md-8 col-md-offset-4"
            ...
            {({ getRootProps, getInputProps, isDragReject, open }) => {
              return (
                <div
                  {...getRootProps()}
                  className={
                    (isDragReject ? "field-validation-error " : "") + "dropzone"
                  }
                  onClick={() => open()}
                >
                  <h3>
                    <span className="mobile">
                      Tap to select descriptive image
                    </span>
                  ...
                  <div className="instructions">
                    <p
                      className={
                        this.state.invalid ? "field-validation-error" : "hidden"
                      }
                    >
                      This image is not valid. Please edit it or pick another
                      one.
                    </p>
                    <p>Use jpg, png, gif or bmp. Max. 1 MB.</p>
                  </div>
                  <input {...getInputProps()} />
                </div>
              );
        ...
        >
          <img
            id="preview"
            src={this.createSrcImage()}
            className="pull-right"
          ></img>
        </div>
        <div
          className="col-xs-12 hidden-md hidden-lg"
          style={{ display: this.state.preview ? "block" : "none" }}
        >
          {this.renderImageControl()}
        </div>
  ...
  private setDescriptionVisibility() {
    if (this.state.preview === true) {
      ...
      this.props.descriptionField.value = "";
    }
  }
}
